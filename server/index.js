...
app.post('/api/vin/decode', (req, res) => {
  ...
  if (vin.length !== 17) return res.status(400).json({ message: 'VIN-код должен содержать 17 символов' });
  const decodedVehicle = { brand: 'Toyota', model: 'Camry', year: '2020' };
  res.json({ vin, vehicle: decodedVehicle, suggestions: products.filter((p) => p.compatibility.some((c) => c.includes(decodedVehicle.brand))).slice(0, 6) });
});
...
app.get('/api/search/suggestions', (req, res) => {
  ...
  const suggestions = products
    .filter((p) => p.name.toLowerCase().includes(q) || p.brand.toLowerCase().includes(q) || p.categoryName.toLowerCase().includes(q))
    .slice(0, 6)
...
app.get('/api/products', (req, res) => {
  const { category, search, vin, sort = 'popular', brands: brandsParam, priceMin = '0', priceMax = '1000000', inStock, rating, oilType, volume, filterType, brand, model, year } = req.query;
  ...
  if (brand && model && year) {
    result = result.filter((p) => p.compatibility.some((line) => line.includes(String(brand)) && line.includes(String(model))));
  }
  result = result.filter((p) => p.price >= Number(priceMin) && p.price <= Number(priceMax));
...
app.post('/api/cart/recommendations', (req, res) => {
  ...
  const hasOil = cartItems.some((i) => String(i.category).includes('Моторное масло') || i.category === 'oils');
  const recommendations = hasOil ? products.filter((p) => p.category === 'filters').slice(0, 4) : products.slice(0, 4);
  res.json({ recommendations, message: hasOil ? 'Для выбранного масла рекомендуем подходящие фильтры.' : 'Популярные дополнения к заказу.' });
});
